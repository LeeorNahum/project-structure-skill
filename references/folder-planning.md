...
`Planning/` is an organizer folder for durable thinking, notes, constraints, and context that does not belong inside a separate repo for finalized deliverables. A planning repo lives directly under `Planning/` as a canonical deliverable folder, usually ending in `-Plan`. The exact name should describe the planned surface or project, with `Docs/` holding the active plan and `README.md` owning purpose and context.

## Initial Shape

```text
Planning/
└── <Project-or-Surface>-Plan/
    ├── AGENTS.md
    ├── README.md
    └── Docs/
```
